...
class ConvAE(keras.Model):
	def __init__(self, encoder, decoder, batch_size, lambdas, gama, init_C=1.0e-8, **kwargs):
		...
		self.lambda_rec, self.lambda_reg, self.lambda_exp, self.lambda_lap1 = lambdas
		self.gama = gama
	...
	def train_step(self, data):
		...
		with tf.GradientTape() as tape:
			...
			total_loss = self.lambda_reg*regularizer_loss + self.lambda_exp*selfexpression_loss + self.lambda_rec*reconstruction_loss + self.lambda_lap1*laplace1_loss
...
def get_best_matrics(images, labels, lambdas, num_epoch, num_epoch_per_print, start_epoch, interval_epoch, init_C, datasetname):
	...
	class CustomCallback(keras.callbacks.Callback):
		def on_epoch_end(self, epoch, logs=None):
			...
			if epoch > start_epoch and epoch % interval_epoch  == 0:
				...
				print("Epoch {}, reconstruction_loss: {}, regularizer_loss: {}, express_loss: {}, laplace1_loss: {}".format(epoch, logs['reconstruction_loss'], logs['regularizer_loss'], logs['selfexpression_loss'], logs['laplace1_loss']))
				...
				print("Epoch {}, reconstruction_loss: {}, regularizer_loss: {}, express_loss: {}, laplace1_loss: {}".format(epoch, logs['reconstruction_loss'], logs['regularizer_loss'], logs['selfexpression_loss'], logs['laplace1_loss']))
			# if epoch % num_epoch_per_print == 0:
			# 	print("Epoch {}, reconstruction_loss: {}, regularizer_loss: {}, express_loss: {}".format(epoch, logs['reconstruction_loss'], logs['regularizer_loss'], logs['selfexpression_loss']))
			# if epoch > start_epoch and epoch % interval_epoch  == 0:	# 1000轮以后，每100论计算一次acc并输出
			# 	Coef = np.array(convAE.Coef)
			# 	acc_nmi_purity = utils.cluster_and_getACC(Coef, labels, num_class=num_class, d=d, alpha=alpha, ro=ro)
			# 	print('epoch: {} 	acc_nmi_purity***********************************************************: {}'.format(epoch, acc_nmi_purity))
			# 	if acc_nmi_purity[0] > best_matrics[0]:
	...
	np.savetxt('Coef_{}_1.csv'.format(datasetname), best_coef) 
...
def test_orl():
	...
	for lambda_rec in [1]:# 1 3 1: 85  1 8 0.5
		for lambda_reg in [8]: # [6, 7]
			for lambda_exp in [1]: # [2.5]
				for lambda_lap1 in [0.0005, 0.00005]: # 0.0001
					lambdas = [lambda_rec, lambda_reg, lambda_exp, lambda_lap1]
	...
	with open('result_lap1', 'a') as file:
	...
	for lambda_rec in [1]:# 1 3 1: 85  1 8 0.5
		for lambda_reg in [1]: # [6, 7]
			for lambda_exp in [20]: # [2.5]
				for lambda_lap1 in [0.0005, 0.00005]: # 0.00001
					lambdas = [lambda_rec, lambda_reg, lambda_exp, lambda_lap1]
					acc_nmi_purity = get_best_matrics(images, labels, lambdas,
	...
	for lambda_rec in [2]:# 1 3 1: 85  1 8 0.5
		for lambda_reg in [1]: # [6, 7]
			for lambda_exp in [30]: # [2.5]
				for lambda_lap1 in [0.05, 0.005]: # 0.01
					lambdas = [lambda_rec, lambda_reg, lambda_exp, lambda_lap1]
					acc_nmi_purity = get_best_matrics(images, labels, lambdas,
	...
	for lambda_rec in [10]:# 1 3 1: 85  1 8 0.5
		for lambda_reg in [8]: # [6, 7]
			for lambda_exp in [12]: # [2.5]
				for lambda_lap1 in [0.1, 0.05, 0.01]: # 0.02
					lambdas = [lambda_rec, lambda_reg, lambda_exp, lambda_lap1]
					acc_nmi_purity = get_best_matrics(images, labels, lambdas,
