...
class ConvAE(keras.Model):
	def __init__(self, encoder, decoder, batch_size, lambdas, gama, init_C=1.0e-8, **kwargs):
		...
		self.lambda_rec, self.lambda_reg, self.lambda_exp = lambdas
		self.gama = gama
	...
	def train_step(self, data):
		...
		with tf.GradientTape() as tape:
			...
			total_loss = self.lambda_reg*regularizer_loss + self.lambda_exp*selfexpression_loss + self.lambda_rec*reconstruction_loss
...
def get_best_matrics(images, labels, lambdas, num_epoch, num_epoch_per_print, start_epoch, interval_epoch, init_C, datasetname):
	...
	np.savetxt('Coef_{}.csv'.format(datasetname), best_coef) 
	...
	for lambda_rec in [1]:# 1 3 1: 85  1 8 0.5
		for lambda_reg in [8]: # [6, 7]
			for lambda_exp in [1]: # [2.5]
				lambdas = [lambda_rec, lambda_reg, lambda_exp]
...
def test_eyaleb():
	...
	best_parameters = []
	for lambda_rec in [1]:# 1 3 1: 85  1 8 0.5
		for lambda_reg in [1]: # [6, 7] 0.001
			for lambda_exp in [20]: # [2.5] 0.2
				lambdas = [lambda_rec, lambda_reg, lambda_exp]
				acc_nmi_purity = get_best_matrics(images, labels, lambdas,
