...
class ConvAE(keras.Model):
	def __init__(self, encoder, decoder, batch_size, lambdas, gama, init_C=1.0e-8, **kwargs):
		...
		self.lambda_rec, self.lambda_reg, self.lambda_exp, self.lambda_lap1, self.lambda_lap2 = lambdas
		self.gama = gama
		...
		self.laplace1_loss_tracker = keras.metrics.Mean(name="laplace1_loss")
	...
	def train_step(self, data):
		...
		with tf.GradientTape() as tape:
			...
			XLX2 = tf.matmul(tf.matmul(tf.transpose(x_flatten), L), reconstruction_flatten)
			laplace1_loss = 2.0 * tf.linalg.trace(XLX2)
			...
			total_loss = self.lambda_reg*regularizer_loss + self.lambda_exp*selfexpression_loss + self.lambda_rec*reconstruction_loss + self.lambda_lap1*laplace1_loss + self.lambda_lap2*laplace2_loss
		...
		return {
			...
			"laplace1_loss": self.laplace1_loss_tracker.result(),
	...
	class CustomCallback(keras.callbacks.Callback):
		def on_epoch_end(self, epoch, logs=None):
			...
			if epoch > start_epoch and epoch % interval_epoch  == 0:
				...
				print("Epoch {}, reconstruction_loss: {}, regularizer_loss: {}, express_loss: {}, laplace1_loss: {}, laplace2_loss: {}".
						format(epoch, logs['reconstruction_loss'], logs['regularizer_loss'], logs['selfexpression_loss'], logs['laplace1_loss'], logs['laplace2_loss']))
...
def test_orl():
	...
	best_matrics = [0, 0, 0]
	best_parameters = []
	for lambda_rec in [1]:# (1, 6, 1, 0.00005, 0.002))
		for lambda_reg in [1000, 100, 10, 1, 0.1]: # [6, 7]
			for lambda_exp in [100, 10, 1, 0.1, 0.01]: # [2.5]
				for lambda_lap1 in [0.00005]: # 
					for lambda_lap2 in [0.002]:
						best_matrics = [0, 0, 0]
						lambdas = [lambda_rec, lambda_reg, lambda_exp, lambda_lap1, lambda_lap2]
						acc_nmi_purity = get_best_matrics(images, labels, lambdas,
									num_epoch=2500, num_epoch_per_print=70, start_epoch=1800, interval_epoch=70, init_C=1.0e-8, datasetname='orl')
						if acc_nmi_purity[0] > best_matrics[0]:
							best_matrics = acc_nmi_purity
							best_parameters = lambdas
						with open('fenxi2.txt', 'a') as file:
							file.write('{} {} {}\n'.format(lambda_reg, lambda_exp, acc_nmi_purity[0]))
	print('best_matrics: {} with best_parameters: {}'.format(best_matrics, best_parameters))
	
	np.savetxt('Coef_{}_both.csv'.format("orl"), best_coef) 
	with open('result_lap_both', 'a') as file:
...
def test_eyaleb():
	...
	for lambda_rec in [1]:# 1 3 1: 85  1 8 0.5
		for lambda_reg in [7]: # [6, 7]
			for lambda_exp in [18]: # [2.5]
				for lambda_lap1 in [0.000008]:
					for lambda_lap2 in [0.00005]:
						lambdas = [lambda_rec, lambda_reg, lambda_exp, lambda_lap1, lambda_lap2]
						acc_nmi_purity = get_best_matrics(images, labels, lambdas,
									num_epoch=5000, num_epoch_per_print=5, start_epoch=2500, interval_epoch=1, init_C=1.0e-8, datasetname='eyaleb')
						if acc_nmi_purity[0] > best_matrics[0]:
							best_matrics = acc_nmi_purity
							best_parameters = lambdas
	print('best_matrics: {} with best_parameters: {}'.format(best_matrics, best_parameters))
	np.savetxt('Coef_{}_both.csv'.format("eyaleb"), best_coef) 
	with open('result_lap_both', 'a') as file:
		file.write('yaleb: \nbest_matrics: {} with best_parameters: {}\n\n'.format(best_matrics, best_parameters))
	...
	for lambda_rec in [0.2]:# 1 3 1: 85  1 8 0.5
		for lambda_reg in [20]: # [6, 7]
			for lambda_exp in [5]: # [2.5]
				for lambda_lap1 in [0.008]: # 0005
					for lambda_lap2 in [0.001]: # 001
						lambdas = [lambda_rec, lambda_reg, lambda_exp, lambda_lap1, lambda_lap2]
						acc_nmi_purity = get_best_matrics(images, labels, lambdas,
						...
						if acc_nmi_purity[0] > best_matrics[0]:
							...
							best_parameters = lambdas
	print('best_matrics: {} with best_parameters: {}'.format(best_matrics, best_parameters))
	np.savetxt('Coef_{}_both.csv'.format('coil20'), best_coef) 
	with open('result_lap_both', 'a') as file:
		file.write('coil20: \nbest_matrics: {} with best_parameters: {}\n\n'.format(best_matrics, best_parameters))
...
def test_umist():
	...
	best_matrics = [0, 0, 0]
	best_parameters = []
	for lambda_rec in [2]:# (2,1,30,0.005,1.5)
		for lambda_reg in [1]: # [6, 7]
			for lambda_exp in [30]: # [2.5]
				for lambda_lap1 in [1, 0.1, 0.01, 0.001, 0.0001]:
					for lambda_lap2 in [0.01]:
						best_matrics = [0, 0, 0]
						lambdas = [lambda_rec, lambda_reg, lambda_exp, lambda_lap1, lambda_lap2]
						acc_nmi_purity = get_best_matrics(images, labels, lambdas,
								num_epoch=700, num_epoch_per_print=10, start_epoch=500, interval_epoch=10, init_C=1.0e-8, datasetname='umist')
						if acc_nmi_purity[0] > best_matrics[0]:
							best_matrics = acc_nmi_purity
							best_parameters = lambdas
						with open('fenxi.txt', 'a') as file:
							file.write('{} {} {}\n'.format(lambda_lap1, lambda_lap2, acc_nmi_purity[0]))
	print('best_matrics: {} with best_parameters: {}'.format(best_matrics, best_parameters))
	np.savetxt('Coef_{}_both.csv'.format('umist'), best_coef) 
	with open('result_lap_both', 'a') as file:
		file.write('umist: \nbest_matrics: {} with best_parameters: {}\n\n'.format(best_matrics, best_parameters))
	...
	for lambda_rec in [10]:# 1 3 1: 85  1 8 0.5
		for lambda_reg in [8]: # [6, 7]
			for lambda_exp in [12]: # [2.5]
				for lambda_lap1 in [0.01]: # 0.01
					for lambda_lap2 in [0.05]: # 0.01
						lambdas = [lambda_rec, lambda_reg, lambda_exp, lambda_lap1, lambda_lap2]
						acc_nmi_purity = get_best_matrics(images, labels, lambdas,
						...
						if acc_nmi_purity[0] > best_matrics[0]:
							...
							best_parameters = lambdas
	print('best_matrics: {} with best_parameters: {}'.format(best_matrics, best_parameters))
	np.savetxt('Coef_{}_both.csv'.format('mnist'), best_coef) 
	with open('result_lap_both', 'a') as file:
		file.write('mnist: \nbest_matrics: {} with best_parameters: {}\n\n'.format(best_matrics, best_parameters))
...
# datasets = ["ORL_config.yaml", 'MNIST_config.yaml', "COIL20_config.yaml"]
datasets = ["ORL_config.yaml"]
